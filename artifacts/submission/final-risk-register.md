...
| 2 | Target 8 GB Ubuntu 22.04 hardware not independently validated | TPS/RAM on the actual target is unknown; dev machine has 32 GB. | Task 005C and Task 005D both ran the full benchmark on macOS (PASS, real TPS captured, no traps), but neither machine is Ubuntu 22.04 / 8 GB. 005D confirms the 8 GB target was attempted but not reachable from this environment. llama.cpp projects ~5.3 GB host usage (fits within 8 GB), but projection ≠ measured free RAM. Gap remains for a true Ubuntu 22.04 run. | Partially mitigated — open |
